...
def index(request):
    data = {
        ...
        'productsData' : Product.objects.order_by('-id')[:4],
    }
...
def contact(request):
    if request.method=="POST":
        ...
        send_mail(subject, message, email, ['avinav.shrestha@pcmgmt.edu.np'])
        return redirect('contact')
    return render(request, 'frontend/pages/contact/contact.html')

def product_list(request):
    productData = Product.objects.order_by('-id')
    paginator = Paginator(productData, 4)  # Show 25 contacts per page.
...
def product_search(request):
    if request.method == 'POST':
        ...
        productsData = Product.objects.filter(Q(name__icontains=criteria))
        paginator = Paginator(productsData, 4)  # Show 25 contacts per page.
        page_number = request.GET.get("page")
        page_obj = paginator.get_page(page_number)
        data = {
            'productsData' : page_obj,
...
def user_dashboard(request):
    ...
    order_data= Order.objects.filter(customer=c_data)
